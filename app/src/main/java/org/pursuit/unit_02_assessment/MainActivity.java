...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            inputEditText.setText(savedInstanceState.getInt("edittext"));
        ...
        randomNumber = randomGame.getRandomNumber();
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        int id = item.getItemId();
        String location = "40.7429595,-73.94192149999998(Pursuit Android HQ)";
        Uri locationUri = Uri.parse("geo:0,0?q=" + location);
        Intent callIntent = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:2125551212"));
        Intent textIntent = new Intent(Intent.ACTION_SENDTO, Uri.parse("sms:2125551212"));
        ...
        switch (id) {
            ...
            case R.id.nav_sms:
                if (textIntent.resolveActivity(getPackageManager()) != null) {
                    textIntent.putExtra("message", "Welcome to Pursuit!");
    ...
    public void submitThings() {
        int checkThisNumber = Integer.parseInt(inputEditText.getText().toString());
        boolean randomCheck = randomGame.checkGuess(checkThisNumber, randomNumber);
        String randomCheckAnswer = randomGame.getStringResult(randomCheck);
        Intent intent = new Intent(this, SecondActivity.class);
        if (randomCheck) {
            intent.putExtra("result", randomCheckAnswer);
            startActivity(intent);
        } else {
            textView.setText(randomCheckAnswer);
        }
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putInt("edittext", Integer.parseInt(inputEditText.getText().toString()));
